...
class user_edit:
	...
	self.SexListStore = gtk.ListStore(gobject.TYPE_INT, gobject.TYPE_STRING)
        ...
        iter3 = self.SexListStore.append((1, "Feminino"))
        iter4 = self.SexListStore.append((2, "Masculino"))
        self.sex_iters[1] = iter3
        self.sex_iters[2] = iter4
        #Occupation
	...
	if address.city:
        ...
        if registro_geral.date_expedicao:
            self.data['date_expedicao'] = registro_geral.date_expedicao
            self.data_exp.set_time(map(int, self.data['date_expedicao'].split('-')))
    ...
    def get_data(self):
        ...
        newdata['date_expedicao'] = '%s-%s-%s' % self.data_exp.get_time()
        ...
        if response == 0:
            ...
            for key, value in data.items():
                ...
                if key == "city" or key == "user" or key == "state" or key == "pais" or key == "address" or key == "cep" or key == "bairro":
                    ...
                    address.id = value
                elif key == "numero_ident" or key == "orgao_expeditor" or key == "date_expedicao":
...
class authentication_dialog:
    ...
    def run(self):
        ...
        if not inf_gerente_login:
            self.lbl_gerente_info.set_text("Para login utilize usuário: gestor / senha: 123 \n e desative essa informação no menu Editar > Preferências.")
