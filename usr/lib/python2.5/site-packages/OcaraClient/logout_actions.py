...
from OcaraCore.utils import is_in_path, execute_command, get_output_of_command

try:
    from OcaraClient import gdm
    HAS_GDM_CLIENT = True
except:
...
class PosixActionManager:
    ...
    def shutdown(self):
        executable = is_in_path("gnome-power-cmd")
        if not executable:
            return False
        
        cmd = [executable, "shutdown"]
        a = execute_command(cmd)
        print a
